...
const token=localStorage.getItem('token')
 ...
 const getData=async()=>{
   setLoading(true)
  const response= await axios.get(`${process.env.REACT_APP_API_KEY}user/getDepartmentList`, 
   { headers: {"token" : `${token}`}})
...
e.target.reset();
  const response= await axios.post(`${process.env.REACT_APP_API_KEY}user/addDeprt`,obj,
   { headers: {"token" : `${token}`}})
   console.log(response.data.data)
...
const obj={departmentName:Department.current.value}
 ...
 const response= await axios.patch(`${process.env.REACT_APP_API_KEY}user/updateDepartment/${id}`,obj,
  { headers: {"token" : `${token}`}})
  console.log(response.data.data)
...
const deleteHandler=async(index)=>{
  ...
  const response= await axios.delete(`${process.env.REACT_APP_API_KEY}user/delete/${data[index]._id}`, 
   { headers: {"token" : `${token}`}})
            ...
            data.map((value, index) => {
              return (
                ...
                <td>{index+1}</td>
                  ...
                  <td onClick={()=>editHandler(index)}><FontAwesomeIcon icon={faEdit}  /> </td>
                  <td onClick= {()=>deleteHandler(index)}><FontAwesomeIcon icon={faTrash}  /></td>
