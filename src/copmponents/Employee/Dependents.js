...
const callApi =async ()=>{
    try {
        let config = {
          ...
          headers: {
            'token': 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjYyZWE2NGIzYzA2NzIwODNmNzI5NDFhOSIsIkFjY291bnQiOiIzIiwiaWF0IjoxNjU5NTg5MzI4fQ.AQG1PcM2lNLpEJA8JFMZfomcXjuMq2dHS6Cy0eXgobw'
          }
        };
  ...
  <table class="table">
        ...
        <tbody>
          {familyMem && familyMem?.data?.map((item, i) => {
            return (
              <>
                <tr key={i}>
                  <th scope="row"> {i + 1}</th>
