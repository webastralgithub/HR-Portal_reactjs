...
const getData=async()=>{
   setLoading(true)
const response= await axios.get(`${process.env.REACT_APP_API_KEY}user/getRole`, 
 { headers: {"token" : `${token}`}})
...
e.target.reset();
 const response= await axios.post(`${process.env.REACT_APP_API_KEY}user/addRole`,obj,
  { headers: {"token" : `${token}`}})
  console.log(response.data.data)
...
const obj={roleType:Role.current.value}
 ...
 const response= await axios.patch(`${process.env.REACT_APP_API_KEY}user/updateRole/${id}`,obj,
  { headers: {"token" : `${token}`}})
  console.log(response.data.data)
...
const deleteHandler=async(index)=>{
  ...
  const response= await axios.delete(`${process.env.REACT_APP_API_KEY}user/deleteRole/${data[index]._id}`, 
   { headers: {"token" : `${token}`}})
