...
const token=localStorage.getItem('token')
 ...
 const getData=async()=>{
     setLoading(true)
  const response= await axios.get(`${process.env.REACT_APP_API_KEY}user/getPositionList`, 
   { headers: {"token" : `${token}`}})
...
e.target.reset();
 const response= await axios.post(`${process.env.REACT_APP_API_KEY}user/addPosition`,obj,
  { headers: {"token" : `${token}`}})
  console.log(response.data.data)
...
const obj={positionName:Position.current.value}
 ...
 const response= await axios.patch(`${process.env.REACT_APP_API_KEY}user/updatePosition/${id}`,obj,
  { headers: {"token" : `${token}`}})
  console.log(response.data.data)
...
const deleteHandler=async(index)=>{
  ...
  const response= await axios.delete(`${process.env.REACT_APP_API_KEY}user/deletePosition/${data[index]._id}`, 
   { headers: {"token" : `${token}`}})
